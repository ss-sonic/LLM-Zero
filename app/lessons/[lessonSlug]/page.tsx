...
import { Utf8Lesson } from "../../../curriculum/07-utf8/lesson";
...
export async function generateMetadata({ params }: { params: Promise<{ lessonSlug: string }> }): Promise<Metadata> { const { lessonSlug } = await params; const lesson = getLessonMeta(lessonSlug); if (!lesson) return {}; return { title: lesson.title, description: lesson.description }; }
...
export default async function LessonPage({ params }: { params: Promise<{ lessonSlug: string }> }) {
  ...
  switch (lessonSlug) {
    case "character-representation": return <CharacterRepresentationLesson />;
    case "shared-character-table": return <SharedCharacterTableLesson />;
    case "ascii": return <AsciiLesson />;
    case "breaking-ascii": return <BreakingAsciiLesson />;
